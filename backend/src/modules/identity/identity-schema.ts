...
export function identitySeedSql(): string {
  ...
  const permissions: string[] = [
    ...
    'contract:sign', 'contract:anchor', 'board:vote', 'trustee:veto',
    'ph:surveillance', 'ph:outbreak-declare',
    'breakglass:request', 'breakglass:approve',
  ];

  const rolePermissions: Record<string, string[]> = {
    trustee: ['audit:read', 'audit:export', 'ai:killswitch', 'trustee:veto', 'contract:sign', 'contract:anchor', 'rbac:read', 'breakglass:approve', 'tenant:switch'],
    board: ['audit:read', 'rbac:read', 'contract:sign', 'board:vote', 'billing:read', 'tenant:switch', 'ai:configure'],
    ceo: ['audit:read', 'audit:export', 'rbac:read', 'rbac:write', 'tenant:switch', 'tenant:admin', 'hr:read', 'hr:write', 'billing:read', 'ai:configure', 'ai:override', 'breakglass:request', 'breakglass:approve', 'contract:sign'],
    cmo: ['patient:read', 'phi:read', 'phi:write', 'rx:write', 'rx:controlled', 'note:write', 'note:sign', 'order:lab', 'order:imaging', 'order:procedure', 'discharge:approve', 'ai:override', 'ai:configure', 'audit:read', 'breakglass:approve'],
    cno: ['patient:read', 'phi:read', 'phi:write', 'note:write', 'note:sign', 'rx:dispense', 'breakglass:approve'],
    cfo: ['billing:read', 'billing:write', 'claim:submit', 'payment:receive', 'audit:read'],
    cto: ['audit:read', 'ai:configure', 'ai:killswitch', 'rbac:read'],
    cro: ['audit:read', 'audit:export', 'rbac:read', 'ai:killswitch'],
    'general-counsel': ['contract:sign', 'contract:anchor', 'audit:read', 'rbac:read'],
    doctor: ['patient:read', 'patient:write', 'phi:read', 'phi:write', 'rx:write', 'order:lab', 'order:imaging', 'order:procedure', 'note:write', 'note:sign', 'discharge:approve', 'ai:override', 'breakglass:request', 'tenant:switch'],
    nurse: ['patient:read', 'phi:read', 'phi:write', 'note:write', 'rx:dispense', 'breakglass:request'],
    pharmacy: ['patient:read', 'phi:read', 'rx:dispense', 'inventory:read', 'inventory:write', 'note:write'],
    'pharmacy-chief': ['patient:read', 'phi:read', 'rx:dispense', 'rx:controlled', 'inventory:read', 'inventory:write', 'po:approve', 'note:write', 'audit:read'],
    lab: ['patient:read', 'phi:read', 'phi:write', 'order:lab', 'note:write', 'inventory:read'],
    radiology: ['patient:read', 'phi:read', 'phi:write', 'order:imaging', 'note:write', 'note:sign'],
    admin: ['patient:register', 'hr:read', 'billing:read', 'audit:read', 'tenant:admin', 'rbac:read', 'inventory:read', 'breakglass:request'],
    'hr-director': ['hr:read', 'hr:write', 'payroll:run', 'audit:read', 'rbac:read', 'contract:sign', 'tenant:switch'],
    finance: ['billing:read', 'billing:write', 'claim:submit', 'payment:receive', 'inventory:read'],
    procurement: ['inventory:read', 'inventory:write', 'po:approve'],
    'ai-safety-officer': ['ai:configure', 'ai:override', 'ai:killswitch', 'audit:read', 'audit:export', 'rbac:read'],
    auditor: ['audit:read', 'audit:export', 'rbac:read', 'billing:read', 'tenant:switch'],
  ...
  for (const [id, label, cadre] of roles) {
    lines.push(`INSERT INTO beyu_identity.roles (role_id, label, cadre) VALUES ('${id}', '${label.replace(/'/g, "''")}', '${cadre}') ON CONFLICT (role_id) DO NOTHING;`);
  }
  for (const p of permissions) {
    lines.push(`INSERT INTO beyu_identity.permissions (permission_id) VALUES ('${p}') ON CONFLICT (permission_id) DO NOTHING;`);
  }
  for (const [role, perms] of Object.entries(rolePermissions)) {
    for (const p of perms) {
      lines.push(`INSERT INTO beyu_identity.role_permissions (role_id, permission_id) VALUES ('${role}', '${p}') ON CONFLICT DO NOTHING;`);
    }
  }
  return lines.join('\n');
}
...
export async function ensureIdentitySchema(conn: { exec(sql: string): Promise<void> }): Promise<void> {
