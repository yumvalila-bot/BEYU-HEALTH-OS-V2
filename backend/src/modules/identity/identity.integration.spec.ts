...
describe('Phase 1A identity persistence (real PostgreSQL via PGlite)', () => {
  ...
  beforeAll(async () => {
    ...
    // Seed tenants.
    const tenantA = await repo.createTenant({ code: 'TENANT-A', name: 'Tenant A' });
    const tenantB = await repo.createTenant({ code: 'TENANT-B', name: 'Tenant B' });
    ...
    await repo.ensureMembership({ globalUserId: doctorA, tenantId: tenantAId, role: 'doctor' });
    await repo.ensureMembership({ globalUserId: nurseA, tenantId: tenantAId, role: 'nurse' });
  ...
  it('enforces unique email at the DB level', async () => {
    await expect(
      repo.createUser({ email: 'unique@example.com', displayName: 'Dup', passwordHash }),
    ).rejects.toThrow();
  });

  it('enforces unique (global_user_id, tenant_id) membership', async () => {
    await expect(
      repo.ensureMembership({ globalUserId: doctorA, tenantId: tenantAId, role: 'doctor' }).then(
        () => repo.ensureMembership({ globalUserId: doctorA, tenantId: tenantAId, role: 'doctor' }),
      ),
  ...
  it('logs in a valid user with correct credentials', async () => {
    const tokens = await auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
  ...
  it('denies a disabled account', async () => {
    const u = await repo.createUser({ email: 'disabled@example.com', displayName: 'D', passwordHash, accountStatus: 'disabled' });
    await repo.ensureMembership({ globalUserId: u.global_user_id, tenantId: tenantAId, role: 'patient' });
    await expect(
      auth.login({ email: 'disabled@example.com', password: PASSWORD, tenantCode: 'TENANT-A' }),
    ).rejects.toThrow('ACCOUNT_DISABLED');
  });

  it('issues JWTs with role, tenant and unique jti claims', async () => {
    const tokens = await auth.login({ email: 'nurse@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
    const decoded = jwt.verify(tokens.accessToken) as { role: string; tenantId: string; jti: string; sub: string };
  ...
  it('rejects an expired/malformed token', async () => {
    ...
    const forged = jwt.sign({ email: 'x', role: 'trustee', tenantId: tenantAId }, { secret: 'WRONG-SECRET' });
    expect(() => jwt.verify(forged)).toThrow();
    // Expired token: simulate verification far in the future → must be rejected.
    const fresh = jwt.sign({ sub: doctorA, role: 'doctor' }, { secret: 'test-secret', expiresIn: '1h' });
    const future = Math.floor(Date.now() / 1000) + 7200;
    expect(() => jwt.verify(fresh, { clockTimestamp: future })).toThrow(/expired/i);
  });

  // ── Sessions / refresh rotation ────────────────────────────────────────────
  it('rotates a refresh token and invalidates the old one', async () => {
    const tokens = await auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
    const rotated = await auth.refreshToken({ refreshToken: tokens.refreshToken });
    expect(rotated.accessToken).toBeTruthy();
    expect(rotated.refreshToken).not.toBe(tokens.refreshToken);
    // Old token now invalid.
    await expect(auth.refreshToken({ refreshToken: tokens.refreshToken })).rejects.toThrow();
  });
  ...
  it('supports global logout (invalidate all sessions)', async () => {
    const t1 = await auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
    const t2 = await auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
    await auth.logoutAll(doctorA);
    await expect(auth.refreshToken({ refreshToken: t1.refreshToken })).rejects.toThrow('SESSION_REUSE_DETECTED');
    await expect(auth.refreshToken({ refreshToken: t2.refreshToken })).rejects.toThrow('SESSION_REUSE_DETECTED');
  });
  ...
  it('denies a user logging into a tenant they have no membership in (cross-tenant login)', async () => {
    ...
    await expect(
      auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-B' }),
  ...
  it('blocks cross-tenant requests in the TenantScopeGuard (deny by default)', () => {
    ...
    const actor: ActorContext = { userId: nurseA, email: 'nurse@a.example', role: 'nurse', permissions: [], tenantId: tenantAId };
    tenant.enterWith(actor);
    const guard = new TenantScopeGuard(tenant);
    const req = { headers: { 'x-tenant-id': tenantBId }, params: {}, query: {} } as any;
  ...
  it('allows tenant:switch holders to move scopes', () => {
    const tenant = new TenantContext();
    const actor: ActorContext = { userId: doctorA, email: 'doctor@a.example', role: 'ceo', permissions: ['tenant:switch'], tenantId: tenantAId };
    tenant.enterWith(actor);
    const guard = new TenantScopeGuard(tenant);
    const req = { headers: { 'x-tenant-id': tenantBId }, params: {}, query: {} } as any;
    const ctx = { switchToHttp: () => ({ getRequest: () => req }) } as any;
  ...
  it('persists audit events with WHO/WHAT/TENANT/RESULT', async () => {
    await auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
  ...
  it('records login failures as audit events', async () => {
    ...
    expect(events.some((e) => e.event_type === 'login_failure' && e.result === 'FAILURE')).toBe(true);
  });

  it('never stores a raw refresh token or password in audit/DB', async () => {
    const tokens = await auth.login({ email: 'doctor@a.example', password: PASSWORD, tenantCode: 'TENANT-A' });
    const session = await repo.findSessionByRefreshHash(sessions.hashToken(tokens.refreshToken));
  ...
  it('MFA fails closed when no provider is connected', async () => {
    await expect(mfa.verifyStepUp(doctorA, 'challenge', 'assertion')).rejects.toThrow();
  });

  it('step-up required gate denies unverified users', async () => {
    await expect(mfa.requireStepUp(doctorA)).rejects.toThrow('STEP_UP_REQUIRED');
  });
