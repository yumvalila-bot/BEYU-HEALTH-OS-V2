...
| 1 | Identity & authentication | ❌ | `auth.service.ts` stub; role-switcher demo | No real sign-in | Critical | 0.0 | Real auth via Supabase Auth + NestJS guards | E2E login/logout, token lifecycle |
| 2 | Authorization (RBAC/ABAC) | ❌ | Client-only `rbac.ts` | Server not enforcing | Critical | 1 | Server guards + policy engine + ABAC | AuthZ matrix tests, tenant-scope tests |
| 3 | Multi-tenancy & isolation | ❌ | Schema sketch; RLS bypassed by service key | Cross-tenant leak | Critical | 1,2 | Adopt full schema, user-context proxy, RLS all tables | Tenant-isolation integration tests |
...
| 5 | Transactional attribution & audit | ❌ | Static `TRANSACTIONS` w/ fake hashes | No real audit | Critical | 1,2 | Audit interceptor, tamper-evident hashing, audit table | Audit capture & integrity tests |
